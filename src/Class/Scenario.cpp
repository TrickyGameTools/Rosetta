...
namespace Slyvina {
	namespace Rosetta {
		namespace Class {
			...
			void CPMC::operator=(bool value) {Ouwe->Parent->Parent->Modified=value;}
			void _CPage::LinkUpdate(ELU FromELU,June19::j19gadget* From) {
			...
			CSLang* _CPage::GLang(String lkey) {
				...
				if (!_Lang.count(lkey)) {
					_Lang[lkey].Parent=this;
					_Lang[lkey].Lang=lkey;
				}
				return &_Lang[lkey];
			}

			String _CPage::ChosenLangName1() { return Parent->Parent->Parent->Parent->Settings->Value("::SCENARIO::", "LANG1"); }
			String _CPage::ChosenLangName2() { return  Parent->Parent->Parent->Parent->Settings->Value("::SCENARIO::", "LANG2"); }
			//}
