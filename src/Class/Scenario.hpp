...
namespace Slyvina { namespace Rosetta { namespace Class {
	//internal class CScenario { // That doesn't work in C++
		...
		class CSLang {
			...
			String CGLCat() { return "::LANG::"+Lang+"::"+Parent->Parent->Tag+TrSPrintF("::%d::",PageIndex()); }
		...
		inline CTag GetTag(String ekey,String tkey) { return GetByIdx(ekey)->GT(tkey); } // internal CTag this[string ekey,string tkey] => this[ekey][tkey];
